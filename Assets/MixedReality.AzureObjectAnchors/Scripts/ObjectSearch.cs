...
public class ObjectSearch : MonoBehaviour
{
	...
	private void HandleObjectAnchorsServiceEvent()
	{
		...
		while (_objectAnchorsEventQueue.TryDequeue(out _event))
			switch (_event.Kind)
			{
				...
				{
					TextLogger.LogRaw(
						$"{EventArgsFormatter(_event.Args)} found, coverage {_event.Args.SurfaceCoverage.ToString("0.00")}");
				...
				{
					TextLogger.LogRaw(
						$"{EventArgsFormatter(_event.Args)} updated, coverage {_event.Args.SurfaceCoverage.ToString("0.00")}");
	...
	private void DrawBoundingBox(IObjectAnchorsServiceEventArgs instance)
	{
		...
		if (!_objectPlacements.TryGetValue(instance.InstanceId, out placement))
		{
			...
			bbox.UpdateBounds(boundingBox.Value.Center, Vector3.Scale(boundingBox.Value.Extents, instance.ScaleChange),
				boundingBox.Value.Orientation, WireframeMaterial);
			...
			Task.Run(async () => {
				...
				catch (Exception ex)
				{
					UnityEngine.WSA.Application.InvokeOnAppThread(
						() => { TextLogger.Log($"Detection failed. Exception message: {ex}"); }, false);
				}
